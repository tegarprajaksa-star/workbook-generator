...
type Proc = {
  code: string; name: string; description: string; inputText: string; outputText: string
  totalSla: string; category: string
  lanesJson: string; stepsJson: string; sopsJson: string; formsJson: string
}
      ...
      if (steps.length > 0) {
        children.push(new Table({
          ...
          rows: [
            new TableRow({
              tableHeader: true,
              children: ['No', 'Tipe', 'Aktivitas', 'Lane', 'SLA', 'Cabang'].map((h) => new TableCell({
                shading: { type: ShadingType.CLEAR, fill: accent },
            ...
            ...steps.map(s => new TableRow({
              children: [
                new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: String(s.order), size: 18 })] })] }),
                new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: s.type, size: 18, bold: s.type === 'GATEWAY' })] })] }),
                new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: s.label, size: 18 })] })] }),
                new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: lanes[s.lane] || '-', size: 18 })] })] }),
                new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: s.sla || '-', size: 18 })] })] }),
                new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: s.branchLabel || '', size: 18, color: s.branchLabel === 'TIDAK' ? 'CC0000' : '008800' })] })] }),
              ],
            })),
          ],
        }))
